...
class Action_Plan extends DB_Object
{
		...
		<table>
			<tbody>
				...
				<tr>
					<th>Groups</th>
					<td>
						When this plan is executed, add the persons / famiy members to these groups:
						<?php
			...
			?>
				<tr>
					...
					<td>
						...
						<label for="default_on_create_family">By default, execute this plan when creating a new family</label>
						</p>
						...
						<label for="default_on_add_person">By default, execute this plan when adding a new person to an existing family</label>
						</p>
					</td>
				</tr>
			</tbody>
	...
	static function getMultiChooser($name, $value_or_context)
	{
		...
		foreach ($plans as $id => $plan) {
			...
			$res[] = '<input type="checkbox" name="'.htmlentities($name).'[]"  value="'.(int)$id.'" id="'.$name.'_'.$id.'" '.($selected ? 'checked="checked"' : '').'><label for="'.$name.'_'.$id.'">'.htmlentities($plan['name']).'</label>';
		}
		return implode("<br /> \n", $res);
	}
