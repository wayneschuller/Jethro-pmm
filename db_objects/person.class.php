...
class Person extends DB_Object
{
	...
	function printFieldValue($name, $value=null)
	{
		...
		if (($name == 'email') && !empty($value) && ($value == $this->values['email'])) {
			echo '<a href="mailto: '.htmlentities($this->values['first_name'].' '.$this->values['last_name'].'<'.$this->values[$name].'>').'">'.htmlentities($this->values[$name]).'</a>';
		} else {
	...
	function printForm($prefix='', $fields=NULL)
	{
		if ($GLOBALS['system']->featureEnabled('PHOTOS') && (is_null($fields) || in_array('photo', $fields))) {
			...
			?>
			<img class="float-right" src="?call=person_photo&personid=<?php echo (int)$this->id; ?>" />
			<?php
		}

		if ($GLOBALS['system']->featureEnabled('DATES') && (is_null($fields) || in_array('photo', $fields))) {
