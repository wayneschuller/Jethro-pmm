...
class roster_view extends db_object
{
	...
	*/
				...
				foreach ($members_to_print as $order_num => $member_details) {
					...
					<tr>
						...
						<td>
							...
							<img src="<?php echo BASE_URL; ?>/resources/arrow_up_thin_black.png" class="icon move-row-up" title="Move this role up" />
							<img src="<?php echo BASE_URL; ?>/resources/arrow_down_thin_black.png" class="icon move-row-down" title="Move this role down" />
							<img src="<?php echo BASE_URL; ?>/resources/cross_red.png" class="icon delete-row" title="Delete this role from the list" />
						</td>
					</tr>
					<?php
				}
	...
	function printView($start_date=NULL, $end_date=NULL, $editing=FALSE, $public=FALSE)
	{
		...
		?>
		<table class="standard highlight-rows roster" border="1" cellspacing="0" cellpadding="1">
						...
						if ($editing && empty($mdetail['readonly'])) {
							...
							foreach (array_get($ddetail['assignments'], $mdetail['role_id'], Array()) as $personid => $vs) {
								if (!$public) {
									...
									if (empty($vs['email'])) $n .= '&nbsp;<img src="'.BASE_URL.'/resources/no-email.gif" style="display:inline" title="No Email Address" />';
									$names[] = $n;
								} else {
									$names[] = nbsp($vs['name']);
								}
		...
		<?php
		if (!$public) $this->_printTableFooter($editing, $public); 
		?>
