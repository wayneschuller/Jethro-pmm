...
class service extends db_object
{
		...
		switch ($fieldname) {
			...
			case 'summary':
				...
				if (!empty($this->values['notes'])) {
					?>
					&nbsp;<span class="clickable no-underline" onclick="$(this).next('div.hidden').toggle()">+</span>
					<div class="hidden smallprint"><?php echo nl2br(htmlentities($this->values['notes'])); ?></div>
					<?php
				}
				break;
			default:
	...
	static function getDisplayFields()
	{
		return Array(
				'topic_title' => 'Topic',
				'bible_all'	=> 'All Bible texts',
				'bible_to_read' => 'Bible texts to be read',
				'bible_to_preach' => 'Bible texts to be preached on',
				'format_title'	=> 'Format',
				'notes'			=> 'Notes',
	...
	function getPersonnelByRoleTitle($role_title)
	{
		...
		$assignments =  $GLOBALS['db']->queryAll($sql, null, null, true);
		$role_ids = Array();
		$names = Array();
