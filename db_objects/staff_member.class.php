		...
		switch ($name) {
			case 'password':
				if (($GLOBALS['user_system']->getCurrentUser('id') == $this->id) || $GLOBALS['user_system']->havePerm(PERM_SYSADMIN)) {
					...
					<p class="field-note">Enter once, then again to confirm. Passwords must be at least 6 characters and contain letters and numbers</p>
					<?php
				} else {
					?>
			...
			case 'restrictions':
				...
				if ($GLOBALS['user_system']->havePerm(PERM_SYSADMIN)) {
					...
					<p class="field-note">If you select congregations or groups here, this user will only be able to see persons who belong to one of the selected congregations or groups - it will be as if those are the only congregations, groups and persons that exist in the system.  Users with congregation or group restrictions cannot add new persons or families.  Changes to restrictions take effect immediately.</p>
					<?php
				} else {
			...
			case 'permissions':
				if ($name == 'permissions') {
					...
					?>
					<div style="color: #ccc; position: absolute; right: 10px; padding: 3px"><?php echo $this->getValue($name) ?></div>
					<?php
				}
				if ($GLOBALS['user_system']->havePerm(PERM_SYSADMIN)) {
					parent::printFieldInterface($name, $prefix);
					?>
					<p class="field-note"><b>Note:</b> Changes to permissions only take effect the next time a user logs in</p>
					<?php
				} else {
					$this->printFieldValue($name);
					?>
