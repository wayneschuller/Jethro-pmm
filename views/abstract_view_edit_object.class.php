...
class Abstract_View_Edit_Object extends View
{
	...
	function printView()
	{
		...
		if ($show_form) {
			?>
			<form method="post" enctype="multipart/form-data" id="edit-<?php echo $this->_editing_type; ?>">
				<input type="hidden" name="edit_object_submitted" value="1" />
				<?php $this->_edited_object->printForm(); ?>
				<input type="submit" value="<?php echo $this->_submit_button_label; ?>" />
			<?php
			if ($this->_on_cancel_view) {
				?>
				<a href="?view=<?php echo $this->_on_cancel_view; ?>&<?php echo $this->_editing_type; ?>id=<?php echo $this->_edited_object->id; ?>"><input type="button" class="cancel" value="Cancel" /></a>
				<?php
			}
			?>
			</form>
