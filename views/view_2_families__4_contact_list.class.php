...
class View_Families__Contact_List extends View
{
	...
	function printView()
	{
		if (!empty($_REQUEST['groupid'])) {
			?>
			<a class="float-right" href="<?php echo build_url(Array('call' => 'contact_list', 'view' => NULL)); ?>">Download HTML file</a>
			<?php
			$this->printResults();
		} else {
			$this->printForm();
	...
	function printForm()
	{
		...
		<table class="standard">
			...
			<tr>
				...
				<label for="all_member_details_0">only their names</label><br />
				<input type="radio" name="all_member_details" value="1" id="all_member_details_1" />
				<label for="all_member_details_1">their contact details, same as for opted-in persons</label>
			</tr>
			<tr>
				<th></th>
	...
	function printResults()
	{
		...
		$sql = '
		select family.id as familyid, family.family_name, family.home_tel, person.*, congregation.long_name as congname
		from family 
		join person on family.id = person.familyid
		join congregation on person.congregationid = congregation.id
		...
		foreach ($res as $familyid => $family_members) {
			...
			foreach ($family_members as $member) {
				if (in_array($member['age_bracket'], $_REQUEST['age_bracket'])) {
					$adults[] = $member;
			...
			?>
			<tr><td colspan="4"><h2><?php echo $first_member['family_name']; ?></h2></td></tr>
			<?php
			if ($first_member['home_tel']) {
				$dummy_family->setValue('home_tel', $first_member['home_tel']);
				echo '<tr><td colspan="4"><h3>';
				$dummy_family->printFieldValue('home_tel');
				echo '</h3></td></tr>';
			}
			foreach ($adults as $adult) {
				...
				?>
				<tr>
					<td><?php echo $adults_use_full ? $adult['first_name'].' '.$adult['last_name'] : $adult['first_name']; ?></td>
					<td><?php echo htmlentities($adult['congname']); ?></td>
					<td><?php if ($all_member_details || in_array($adult['id'], $signups)) echo $dummy_person->printFieldValue('mobile_tel'); ?></td>
					<td><?php if ($all_member_details || in_array($adult['id'], $signups)) echo $adult['email']; ?></td>
				</tr>
				<?php
			}
			$child_names = Array();
			...
			if ($child_names) {
				?>
				<tr>
					<td colspan="4"><?php echo implode(', ', $child_names); ?></td>
				</tr>
				<?php
			}
			?>
