...
class View_Persons__List_All extends View
{
	...
	function processView()
	{
		...
		if (empty($_SESSION['total_persons'])) {
			$_SESSION['total_persons'] = $GLOBALS['db']->queryOne('SELECT count(*) from _person');
		}
	...
	{
		if (empty($_REQUEST['show_archived'])) {
			echo '<p class="float-right"><a href="'.build_url(Array('show_archived' => 1)).'">Include Archived Persons</a></p>';
		} else {
			echo '<p class="float-right"><a href="'.build_url(Array('show_archived' => NULL)).'">Exclude Archived Persons</a></p>';
		}
